...
function Header() {
  ...
  return (
    <header className='z-[999] relative'>
        ...
        backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.5rem] sm:w-[36rem] sm:rounded-full' 
            ...
            text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5'>
                {links.map(link =>(
                    ...
                    animate={{ y:0, opacity: 1 }}>
                        ...
                        } href={link.hash} onClick={() => setCurrentSection(link.name)}>
                            {link.name}

                            {
                                link.name === currentSection && (<motion.span className='bg-gray-100 rounded-full absolute inset-0 -z-10'
